...
def calculate_distribution():
    ...
    for b, column in enumerate(columns):
        ...
        seaborn.histplot(
            data=column_data, 
            bins=(math.floor(math.log2(len(column_data))) + 1),
            ax=chart
        )

        chart.set_title(f"{column}")
        chart.set_ylabel("Observações")
        chart.set_xlabel(f"Média: {mean:.2f} - Mediana: {mid:.2f} - Moda: {modes} - Desvio {derivation:.2f}")
        chart.grid(True, axis="y")
        chart.set_yticks(numpy.arange(0, 31, 5))
